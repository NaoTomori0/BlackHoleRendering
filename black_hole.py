    ...
    for i in prange(RES):
        for j in range(RES):
            ...
            for s in range(STEPS):
                ...
                if py * py_next <= 0:
                    ...
                    if 6.0 < r2_d < 300.0:
                        ...
                        radial_boost = 15.0 / (rv**1.3 + 1.0)
                        dop = 1.0 + 0.6 * np.sin(ang)
                        val = (1 + wave) * dop * radial_boost * np.exp(-0.05 * rv)
            ...
            if hit_hole:
                ...
                ps = np.exp(-20 * (min_r - 1.01) ** 2) * 3.0
                glow = np.exp(-1.0 * (min_r - 1.2) ** 2) * 0.4
                b = inst * 0.05 + ps * 0.8
                g = inst * 0.7 + ps * 0.6 + glow * 0.2
                r_c = inst * 2.1 + ps + glow
